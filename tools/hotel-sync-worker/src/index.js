...
export async function run(env){
  ...
  const res=await call(base+'/api/hotel/cron',{method:'POST',headers:{Authorization:'Bearer '+env.HOTEL_CRON_SECRET,'Content-Type':'application/json'},body:JSON.stringify({action:'maintenance'})});
  const body=await res.json().catch(()=>({}));
  if(!res.ok)throw new Error('hotel-cron-http-'+res.status+':'+String(body.error||'unknown'));
  return body;
}
export default{
  async scheduled(_controller,env,ctx){ctx.waitUntil(run(env).then((result)=>console.log(JSON.stringify({event:'hotel-maintenance',sync:result.sync&&{processed:result.sync.processed,failed:result.sync.failed},cleanup:result.cleanup&&{processed:result.cleanup.processed,failed:result.cleanup.failed,deleted:result.cleanup.deleted}}))).catch((error)=>console.error('[hotel-maintenance]',error.message)));},
