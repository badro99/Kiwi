...
- **What it is:** A merchant-controlled digital stamp card: each POS ticket or QR scan drops a stamp, a customer view (kiwi-os.com/carte) shows the card filling, and the reward applies at the caisse when complete. Today's version is a dead mockup (`features.js:1239` — hardcoded stamp dots, a radio-button model picker, a success toast, zero per-customer state); this gives it real per-customer state and persistence.
...
- **Blad — la couche de découverte du quartier** (Consumer/H2) — A consumer map/feed of Kiwi merchants: diners open kiwi-os.com/blad, see nearby cafés/épiceries/pressings, their live 'ouvert maintenant' status, menu, QR-order link and the happy-hour just fired from CRM — the demand side that flips Kiwi into a two-sided marketplace and an anti-Glovo flywheel that compounds with density per quartier.
