...
import { PASSWORD_MAX, verifyPassword, makeSession, sessionCookie, json, normEmail, limitCheck, limitFail, limitClear, targetKey, rateLimitUnavailable } from './_lib.js';
...
export async function onRequestPost(context) {
  ...
  // Reject hostile oversized inputs before a DB lookup or limiter storage read.
  // These are malformed bounds, not credential attempts.
  ...
  const failBoth = async () => {
    const sourceOk = await limitFail(request, env, 'login');
    const targetOk = !target || await limitFail(request, env, 'login', target);
    return sourceOk && targetOk;
  };

  let row;
  try {
    row = await env.DB.prepare('SELECT id, salt, hash, session_epoch FROM accounts WHERE email = ?').bind(email).first();
  } catch (_) {
  ...
  const token = await makeSession(row.id, secret, row.session_epoch);
