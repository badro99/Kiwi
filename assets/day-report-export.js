...
(function () {
  ...
  function build(report, chosen) {
    ...
    if(chosen.summary){section(out,'SYNTHÈSE',['Indicateur','Valeur','Unité']);[['Ouverture',hm(report.openedAt||report.firstSaleAt),''],['Fermeture',hm(report.closedAt),''],['Ouvert par',report.openedBy||'Non renseigné',''],['Fermé par',report.closedBy||'Non renseigné',''],['Net après remboursements',raw(report.net),'MAD'],['Ventes avant remboursements',raw(report.gross),'MAD'],['Créances',raw(report.receivable),'MAD'],['Transactions',report.txns,''],['Ticket moyen',raw(report.basket),'MAD'],['Couverture du détail produit',num(report.coverage),'%']].forEach(function(x){out.push(q(x));});}
