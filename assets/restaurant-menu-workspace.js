...
(function () {
  ...
  function card(x){
    ...
    return `<article class="mi-card${x.avail===false?' rmw-off':''}${x.archived?' rmw-archived':''}" data-rmw-card-edit="${x.id}" role="button" tabindex="0" aria-label="${esc(ui('editItemTitle',{name:xName}))}">${media}<div class="mi-card-top"><span class="mi-card-cat" title="${esc(cName+(subName?` · ${subName}`:''))}">${esc(subName||cName)}</span>${formulaBadge}${badges}${actionMenu}</div><div class="mi-card-name">${esc(xName)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(sName)}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`} · ${nutritionPill(x)}</span><span class="mi-card-acts"><button class="mi-state-toggle" data-rmw-command="availability" data-arg="${x.id}" role="switch" aria-checked="${x.avail!==false}"${x.archived?' disabled':''}>${x.avail===false?'86':'Disponible'}</button></span></div></article>`;
  }
