...
(function () {
  ...
  function card(x){
    ...
    return `<article class="mi-card${x.avail===false?' rmw-off':''}${x.archived?' rmw-archived':''}${menuOpen?' rmw-menu-open':''}"><button type="button" class="rmw-card-edit-hit" data-action="rmw-item-edit" data-arg="${x.id}" aria-label="${esc(ui('editItemTitle',{name:xName}))}"></button>${media}<div class="mi-card-top"><span class="mi-card-cat">${esc(cName)}${subName?` · ${esc(subName)}`:''}</span>${formulaBadge}${badges}${actionMenu}</div><div class="mi-card-name">${esc(xName)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(sName)}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`}</span><span class="mi-card-acts"><button class="mi-state-toggle" data-action="rmw-item-avail" data-arg="${x.id}" role="switch" aria-checked="${x.avail!==false}"${x.archived?' disabled':''}>${x.avail===false?'86':'Disponible'}</button>${x.formula?`<button class="btn-slim" data-action="rmw-formula-duplicate" data-arg="${x.id}">${ic('plus')} Réutiliser</button>`:''}</span></div></article>`;
  }
