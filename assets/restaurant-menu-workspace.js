...
(function () {
  ...
  const lang=()=>(window.KiwiMenuI18n?.lang?.())||(window.KiwiI18n?.getLang?.())||globalThis.localStorage?.getItem?.('kiwiLang')||'fr';
  ...
  const UI={
    ...
    archive:{fr:'Archiver',en:'Archive',ar:'أرشف'},
    restore:{fr:'Restaurer',en:'Restore',ar:'رجع'},
    edit:{fr:'Modifier',en:'Edit',ar:'عدّل'},
    archivedGroup:{fr:'Articles archivés',en:'Archived items',ar:'المنتجات المؤرشفة'},
  };
  const ui=(key)=>UI[key]?.[lang()]||UI[key]?.fr||key;
  const icons={menu:'menu',station:'cooking-pot',book:'book-open',chart:'bar-chart-3',clock:'clock',alert:'alert-triangle',tag:'tag',plus:'plus',edit:'pencil',trash:'trash-2',archive:'archive',restore:'archive-restore'};
  ...
  function card(x){
    ...
    return `<article class="mi-card${x.avail===false?' rmw-off':''}${x.archived?' rmw-archived':''}" data-action="rmw-item-edit" data-arg="${x.id}">${media}<div class="mi-card-top"><span class="mi-card-cat">${esc(cName)}${subName?` · ${esc(subName)}`:''}</span>${formulaBadge}${badges}</div><div class="mi-card-name">${esc(xName)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(sName)}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`}</span><span class="mi-card-acts"><button class="mi-state-toggle" data-action="rmw-item-avail" data-arg="${x.id}" role="switch" aria-checked="${x.avail!==false}"${x.archived?' disabled':''}>${x.avail===false?'86':'Disponible'}</button>${x.formula?`<button class="btn-slim" data-action="rmw-formula-duplicate" data-arg="${x.id}">${ic('plus')} Réutiliser</button>`:''}<button class="mi-ic-btn" data-action="rmw-item-edit" data-arg="${x.id}" aria-label="${esc(stateUi('edit','Modifier'))}" title="${esc(stateUi('edit','Modifier'))}">${ic('edit')}</button><button class="mi-ic-btn" data-action="rmw-item-archive" data-arg="${x.id}" aria-label="${esc(archiveLabel)}" title="${esc(archiveLabel)}">${ic(x.archived?'restore':'archive')}</button></span></div></article>`;
  }
  ...
  function openItem(x){const d=D();if(!d.cats.length){openCategory(null);return;}let photo=x?.photo||'',video=x?.video||'';const availableItems=(d.items||[]).filter(it=>(!x||it.id!==x.id)&&!it.archived);const archivedItems=(d.items||[]).filter(it=>it&&it.archived&&(!x||it.id!==x.id));
...
<div class="rmw-formula-builder" data-formula-wrap ${formulaSlots.length?'':'hidden'} style="margin-top:10px;display:grid;gap:10px;"><div data-slots-wrap style="display:grid;gap:10px;"></div><button type="button" class="btn-slim" data-add-slot style="justify-content:center;">${ic('plus')} Ajouter une étape</button>${archivedItems.length?`<details class="rmw-archived-group"><summary>${esc(ui('archivedGroup'))} · ${archivedItems.length}</summary>${archivedItems.map(it=>`<button type="button" disabled>${esc(it.name)}</button>`).join('')}</details>`:''}</div></div>`,foot:(x?`<button class="kb ghost danger" data-archive>${esc(x.archived?ui('restore'):ui('archive'))}</button>`:'')+'<button class="kb ghost" data-cancel>Annuler</button><button class="eq-cta-gradient" data-save>Enregistrer</button>'});const isFormulaCb=$('[data-is-formula]',m.el),formulaWrap=$('[data-formula-wrap]',m.el),slotsWrap=$('[data-slots-wrap]',m.el);const renderSlots=()=>{if(!slotsWrap)return;slotsWrap.innerHTML=formulaSlots.map(slotRowHtml).join('')||'<span class="kf-help">Aucune étape. Cliquez sur « Ajouter une étape ».</span>';};
