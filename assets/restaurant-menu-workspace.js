...
(function () {
  ...
  const UI_I18N = {
    en: {
      ...
      reuse: 'Reuse',
      standalone: 'Sellable individually',
      formulaOnly: 'Unticked, the item is reserved for set menus and no longer appears on its own on the menu.',
      formulaChoicePhoto: 'Show photo in set menus',
      formulaChoicePhotoHelp: 'In OrderPro, this item’s uploaded photo appears when it is offered as a set-menu choice.',
      archive: 'Archive',
      restore: 'Restore',
      archived: 'Archived',
      formulaOnlyBadge: 'Set menus only',
      step: 'Step',
      extra: 'Extra',
      deleteStep: 'Delete this step',
      removeChoice: 'Remove this choice',
      selectionRule: 'Selection rule',
      archivedGroup: 'Archived items',
      formula: 'SET MENU',
      ...
      nutritionAwaitsRecipe: 'Computed as soon as the recipe has ingredients with quantities.',
      ...
      nutritionNone: 'none',
      nutritionIndicative: 'Indicative values per portion, excluding options',
      nutritionHide: 'Hide nutrition for this dish',
      nutritionHideHint: 'The recipe stays calculated, but customers will not see these values.',
      allergen_gluten: 'Gluten', allergen_crustaces: 'Crustaceans', allergen_oeufs: 'Eggs', allergen_poissons: 'Fish',
      allergen_arachides: 'Peanuts', allergen_soja: 'Soy', allergen_lait: 'Milk', allergen_fruits_a_coque: 'Nuts',
      allergen_celeri: 'Celery', allergen_moutarde: 'Mustard', allergen_sesame: 'Sesame', allergen_sulfites: 'Sulphites',
      allergen_lupin: 'Lupin', allergen_mollusques: 'Molluscs',
      perfTitle: 'Item performance',
      perfSub: 'Popularity, revenue and gross profit · Last 30 days · Real POS data.',
      daysCount: '{n} days',
      rangeLabel: '{from} to {to} · business days',
      menuItemsOnly: 'current menu items only',
      freeAmountItem: 'Free-amount item',
      freeAmountSection: 'Free amount · not on the menu',
      offMenuSection: 'Not on the current menu',
      freeAmountReadout: 'No recipe possible',
      offMenuReadout: 'Removed, renamed or unmatched item',
      ledgerGross: 'Gross sales',
      ledgerTickets: '{n} tickets',
      ledgerRefunds: 'Refunds',
      ledgerRefundCount: '{n} refunds',
      ledgerNet: 'Net revenue',
      ledgerNetSub: 'gross sales minus refunds',
      ledgerEligible: 'Recipe-eligible revenue',
      ledgerEligibleSub: '{share} of gross · menu items only',
      ledgerLoading: 'Sales history is still loading on this device. These figures are partial, not a complete 30 days.',
      ledgerUnknown: 'This device cannot confirm the sales history is complete. Treat these figures as partial.',
      ledgerDuplicates: '{n} duplicate settlement(s) excluded ({amount}): the same ticket arrived more than once with the same time, amount and items.',
      ledgerOffMenu: '{amount} of item lines are outside the current menu, including {free} in free-amount items. They count in sales but cannot have a recipe.',
      ledgerUnitemised: '{amount} of sales is not explained by item lines (ticket discounts, service charges, or {n} tickets without detail).',
      ledgerRefundLines: 'Refunds reduce net revenue. They are not removed item by item from the table below.',
      ...
      analyzedRevenue: 'Menu item revenue',
    ...
    ar: {
      ...
      reuse: 'إعادة استخدام',
      standalone: 'يُباع منفرداً',
      formulaOnly: 'عند إلغاء التحديد يُحصر الصنف في القوائم المجمعة ولا يظهر منفرداً في القائمة.',
      formulaChoicePhoto: 'عرض الصورة داخل الفورمولات',
      formulaChoicePhotoHelp: 'في OrderPro، تظهر صورة هذا الصنف عندما يكون اختياراً داخل فورمولة.',
      archive: 'أرشفة',
      restore: 'استعادة',
      archived: 'مؤرشف',
      formulaOnlyBadge: 'للقوائم المجمعة فقط',
      step: 'المرحلة',
      extra: 'إضافة',
      deleteStep: 'حذف هذه المرحلة',
      removeChoice: 'إزالة هذا الخيار',
      selectionRule: 'قاعدة الاختيار',
      archivedGroup: 'أصناف مؤرشفة',
      formula: 'قائمة مجمعة',
      ...
      nutritionAwaitsRecipe: 'تُحسب بمجرد أن تحتوي الوصفة على مكوّنات بكميات.',
      ...
      nutritionNone: 'لا يوجد',
      nutritionIndicative: 'قيم تقريبية لكل حصة، بدون الخيارات',
      nutritionHide: 'إخفاء القيم الغذائية لهذا الطبق',
      nutritionHideHint: 'تبقى الوصفة محسوبة، لكن الزبائن لن يروا هذه القيم.',
      allergen_gluten: 'الغلوتين', allergen_crustaces: 'القشريات', allergen_oeufs: 'البيض', allergen_poissons: 'السمك',
      allergen_arachides: 'الفول السوداني', allergen_soja: 'الصويا', allergen_lait: 'الحليب', allergen_fruits_a_coque: 'المكسرات',
      allergen_celeri: 'الكرفس', allergen_moutarde: 'الخردل', allergen_sesame: 'السمسم', allergen_sulfites: 'الكبريتيت',
      allergen_lupin: 'الترمس', allergen_mollusques: 'الرخويات',
      perfTitle: 'أداء المنتجات',
      perfSub: 'الشعبية، رقم المعاملات والربح الإجمالي · آخر 30 يوماً · بيانات حقيقية من الصندوق.',
      daysCount: '{n} يوماً',
      rangeLabel: 'من {from} إلى {to} · أيام العمل',
      menuItemsOnly: 'منتجات القائمة الحالية فقط',
      freeAmountItem: 'منتج بمبلغ حر',
      freeAmountSection: 'مبلغ حر · خارج القائمة',
      offMenuSection: 'خارج القائمة الحالية',
      freeAmountReadout: 'لا يمكن ربطه بوصفة',
      offMenuReadout: 'منتج محذوف أو معاد تسميته أو غير مطابق',
      ledgerGross: 'المبيعات الإجمالية',
      ledgerTickets: '{n} تذكرة',
      ledgerRefunds: 'المبالغ المستردة',
      ledgerRefundCount: '{n} استرداد',
      ledgerNet: 'صافي المداخيل',
      ledgerNetSub: 'المبيعات الإجمالية ناقص المستردات',
      ledgerEligible: 'مداخيل قابلة لتحليل الوصفات',
      ledgerEligibleSub: '{share} من الإجمالي · منتجات القائمة فقط',
      ledgerLoading: 'سجل المبيعات ما زال قيد التحميل على هذا الجهاز. هذه الأرقام جزئية وليست 30 يوماً كاملة.',
      ledgerUnknown: 'لا يمكن لهذا الجهاز تأكيد اكتمال سجل المبيعات. اعتبر هذه الأرقام جزئية.',
      ledgerDuplicates: 'تم استبعاد {n} تسوية مكررة ({amount}): نفس التذكرة وصلت أكثر من مرة بنفس الوقت والمبلغ والمنتجات.',
      ledgerOffMenu: '{amount} من سطور المنتجات خارج القائمة الحالية، منها {free} بمبلغ حر. تحتسب في المبيعات لكن لا يمكن ربطها بوصفة.',
      ledgerUnitemised: '{amount} من المبيعات غير مفسرة بسطور المنتجات (تخفيضات على التذكرة، رسوم خدمة، أو {n} تذكرة بدون تفاصيل).',
      ledgerRefundLines: 'المستردات تنقص صافي المداخيل ولا تحذف منتجاً بمنتج من الجدول أدناه.',
    ...
    fr: {
      ...
      reuse: 'Réutiliser',
      standalone: 'Vendable individuellement',
      formulaOnly: 'Décoché, l’article est réservé aux formules et n’apparaît plus seul sur la carte.',
      formulaChoicePhoto: 'Afficher la photo dans les formules',
      formulaChoicePhotoHelp: 'Dans OrderPro, la photo de cet article apparaît lorsqu’il devient un choix de formule.',
      archive: 'Archiver',
      restore: 'Restaurer',
      archived: 'Archivé',
      formulaOnlyBadge: 'Réservé aux formules',
      step: 'Étape',
      extra: 'Supplément',
      deleteStep: 'Supprimer cette étape',
      removeChoice: 'Retirer ce choix',
      selectionRule: 'Règle de sélection',
      archivedGroup: 'Articles archivés',
      formula: 'FORMULE',
      ...
      nutritionAwaitsRecipe: 'Calculée dès que la recette a des ingrédients avec leurs quantités.',
      ...
      nutritionNone: 'aucun',
      nutritionIndicative: 'Valeurs indicatives par portion, hors options',
      nutritionHide: 'Masquer la nutrition pour ce plat',
      nutritionHideHint: 'La recette reste calculée, mais les clients ne verront pas ces valeurs.',
      allergen_gluten: 'Gluten', allergen_crustaces: 'Crustacés', allergen_oeufs: 'Œufs', allergen_poissons: 'Poissons',
      allergen_arachides: 'Arachides', allergen_soja: 'Soja', allergen_lait: 'Lait', allergen_fruits_a_coque: 'Fruits à coque',
      allergen_celeri: 'Céleri', allergen_moutarde: 'Moutarde', allergen_sesame: 'Sésame', allergen_sulfites: 'Sulfites',
      allergen_lupin: 'Lupin', allergen_mollusques: 'Mollusques',
      ...
      daysCount: '{n} jours',
      rangeLabel: 'Du {from} au {to} · journées commerciales',
      menuItemsOnly: 'plats de la carte actuelle uniquement',
      freeAmountItem: 'Article libre',
      freeAmountSection: 'Montant libre · hors carte',
      offMenuSection: 'Hors de la carte actuelle',
      freeAmountReadout: 'Aucune recette possible',
      offMenuReadout: 'Plat retiré, renommé ou non reconnu',
      ledgerGross: 'Ventes brutes',
      ledgerTickets: '{n} tickets',
      ledgerRefunds: 'Remboursements',
      ledgerRefundCount: '{n} remboursements',
      ledgerNet: 'Chiffre d’affaires net',
      ledgerNetSub: 'ventes brutes moins remboursements',
      ledgerEligible: 'CA analysable en recettes',
      ledgerEligibleSub: '{share} des ventes brutes · plats de la carte',
      ledgerLoading: 'L’historique des ventes est encore en cours de chargement sur cet appareil. Ces chiffres sont partiels, pas 30 jours complets.',
      ledgerUnknown: 'Cet appareil ne peut pas confirmer que l’historique des ventes est complet. Considérez ces chiffres comme partiels.',
      ledgerDuplicates: '{n} règlement(s) en double exclu(s) ({amount}) : le même ticket est arrivé plusieurs fois, à la même milliseconde, avec le même montant et les mêmes articles.',
      ledgerOffMenu: '{amount} de lignes sont hors de la carte actuelle, dont {free} en articles libres. Elles comptent dans les ventes mais ne peuvent pas avoir de recette.',
      ledgerUnitemised: '{amount} de ventes ne sont pas expliqués par les lignes d’articles (remises au ticket, frais de service, ou {n} tickets sans détail).',
      ledgerRefundLines: 'Les remboursements réduisent le chiffre net. Ils ne sont pas retirés plat par plat du tableau ci-dessous.',
      ...
      analyzedRevenue: 'CA des plats analysés',
  ...
  function card(x){
    ...
    const u=(typeof ui==='function'?ui:(k,p)=>k==='formula'?'FORMULE':k==='stepCount'?`${p?.n||0} étape(s)`:k==='optGroupCount'?`${p?.n||0} groupe(s) d’options`:k==='noSection'?'Sans section':k==='reuse'?'Réutiliser':k==='editItemTitle'?`Modifier ${p?.name||''}`.trim():k);
    ...
    const stateUi=(key,fallback)=>(typeof ui==='function'?ui(key):fallback);const badges=[x.formulaOnly?stateUi('formulaOnlyBadge','Réservé aux formules'):'',x.archived?stateUi('archived','Archivé'):'',x.avail===false?'86':''].filter(Boolean).map(label=>`<span class="mi-tag">${esc(label)}</span>`).join('');
    const menuOpen = typeof openItemMenu !== 'undefined' && openItemMenu === x.id;
    /* Déclarée ici, juste avant son unique usage : le menu d'actions de la
       fiche l'interpolait sans qu'elle existe nulle part, et un ReferenceError
       levé dans un littéral de gabarit fait échouer le rendu de CHAQUE fiche —
       d'où une page Menu entièrement blanche. On reprend stateUi, déclaré deux
       lignes plus haut, qui porte déjà un libellé de repli si ui() manque. */
    const archiveLabel=x.archived?stateUi('restore','Restaurer'):stateUi('archive','Archiver');
    const actionMenu=`<details class="rmw-card-menu" data-rmw-menu-root><summary class="rmw-more-btn" aria-label="Plus d’actions">${ic('more')}</summary><span class="rmw-action-pop" role="menu">${x.formula?`<button data-rmw-command="duplicate" data-arg="${x.id}" role="menuitem">${ic('plus')}<span>${esc(stateUi('reuse','Réutiliser'))}</span></button>`:''}<button data-rmw-command="archive" data-arg="${x.id}" role="menuitem">${ic(x.archived?'restore':'archive')}<span>${esc(archiveLabel)}</span></button><button class="danger" data-rmw-command="delete" data-arg="${x.id}" role="menuitem">${ic('trash')}<span>${esc(stateUi('delete','Supprimer'))}</span></button></span></details>`;
    return `<article class="mi-card${x.avail===false?' rmw-off':''}${x.archived?' rmw-archived':''}" data-rmw-card-edit="${x.id}" role="button" tabindex="0" aria-label="${esc(u('editItemTitle',{name:xName}))}">${media}<div class="mi-card-top"><span class="mi-card-cat" title="${esc(cName+(subName?` · ${subName}`:''))}">${esc(subName||cName)}</span>${formulaBadge}${badges}${actionMenu}</div><div class="mi-card-name">${esc(xName)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(sName)}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`}${nutritionCardPill(x)}</span><span class="mi-card-acts"><button class="mi-state-toggle" data-rmw-command="availability" data-arg="${x.id}" role="switch" aria-checked="${x.avail!==false}"${x.archived?' disabled':''}>${x.avail===false?'86':'Disponible'}</button></span></div></article>`;
  }
  ...
  function menuPanel(){
    ...
    const searching=!!query.trim(),resultLabel=searching?`${list.length} / ${d.items.length}`:`${d.items.length}`;
    const searchIcon='<svg viewBox="0 -960 960 960" aria-hidden="true"><path d="M784-120 532-372q-30 24-69 38t-83 14q-109 0-184.5-75.5T120-580q0-109 75.5-184.5T380-840q109 0 184.5 75.5T640-580q0 44-14 83t-38 69l252 252-56 56ZM380-400q75 0 127.5-52.5T560-580q0-75-52.5-127.5T380-760q-75 0-127.5 52.5T200-580q0 75 52.5 127.5T380-400Z"/></svg>';
    return `<section class="mi-section"><div class="mi-toolbar rmw-search-toolbar"><label class="rmw-menu-search${searching?' has-query':''}"><span class="rmw-menu-search-icon">${searchIcon}</span><input type="search" value="${esc(query)}" placeholder="${esc(ui('searchPlaceholder'))}" aria-label="${esc(ui('searchPlaceholder'))}" autocomplete="off" data-rmw-search/><span class="rmw-menu-search-count" aria-live="polite"><strong>${resultLabel}</strong><small>${searching?'résultats':'articles'}</small></span></label></div>${cats}${subChips()}${body}</section><section class="mi-section"><div class="mi-section-head"><h3>${esc(ui('modAndOpts'))}</h3><button class="btn-slim primary" data-action="rmw-group-add">${ic('plus')} ${esc(ui('newGroupBtn').replace(/^\+\s*/,''))}</button></div><div class="mi-groups-grid">${d.opts.map(groupCard).join('')||`<div class="rmw-empty">${esc(ui('noOptGroups'))}</div>`}</div></section>`;
  }
  ...
  function nutritionPill(x){const [cls,label]=nutritionStatus(x);return `<span class="mi-group-card-pill rmw-nutrition-pill ${cls}">${esc(label)}</span>`;}
  /* Sur la carte, la place manque (le pied porte déjà « N groupe(s) d’options » et
     le bouton Disponible) : un point coloré + un mot, l’état complet en info-bulle ;
     quand la fiche est complète on affiche les kcal, plus utiles que le mot.
     Rien du tout si l’opérateur a coupé la nutrition pour ce commerçant. */
  function nutritionCardPill(x){if(!menuNutritionFeature())return'';const [cls,label]=nutritionStatus(x);const kcal=x.nutritionComplete===true&&x.nutrition&&Number.isFinite(+x.nutrition.kcal)?Math.round(+x.nutrition.kcal):null;const text=kcal!=null?`${kcal} kcal`:ui('nutritionShort');return ` · <span class="rmw-nutri-dot ${cls}" title="${esc(label)}"><i></i>${esc(text)}</span>`;}
  ...
  /* ── La période analysée ──
   * « 30 derniers jours » = les 30 journées COMMERCIALES qui finissent
   * aujourd'hui, dans le fuseau et avec le seuil de journée du commerce
   * (KiwiDayReport : Africa/Casablanca, 5 h par défaut). L'ancienne fenêtre
   * glissait sur now − 30×24 h dans le fuseau du navigateur : elle coupait une
   * journée en deux et ne correspondait à aucun rapport journalier. */
  function analysisWindow(days=30){
    const DR=window.KiwiDayReport,now=Date.now();
    try{
      if(DR?.today&&DR?.dayBounds&&DR?.shiftDay){
        const last=DR.today(),first=DR.shiftDay(last,-(days-1)),from=DR.dayBounds(first).from,end=DR.dayBounds(last).to;
        if(Number.isFinite(from)&&Number.isFinite(end)&&end>from)return{from,to:Math.min(end,now+5*60e3),first,last,zone:DR.timezone?.()||'Africa/Casablanca',days,exact:true};
      }
    }catch(_){}
    return{from:now-days*864e5,to:now,first:'',last:'',zone:'',days,exact:false};
  }
  function rangeLabel(w){
    if(!w.exact)return ui('daysCount',{n:w.days});
    const fmt=(day)=>{const [y,m,d]=day.split('-').map(Number);return new Intl.DateTimeFormat(lang()==='ar'?'ar-MA':lang()==='en'?'en-GB':'fr-FR',{day:'numeric',month:'short',year:'numeric',timeZone:'UTC'}).format(new Date(Date.UTC(y,m-1,d)));};
    return ui('rangeLabel',{from:fmt(w.first),to:fmt(w.last)});
  }
  /* Le flux du tableau de bord ne sert l'historique qu'au rythme de ses pages
   * (curseur). Tant que la reprise n'a pas atteint sa dernière page, la liste
   * locale n'est qu'un extrait : on le dit au lieu de l'appeler « 30 jours ». */
  function feedCompleteness(){
    try{const st=window.KiwiLive?.status?.();if(!st)return 'unknown';return st.backfillComplete?'complete':'loading';}catch(_){return 'unknown';}
  }
  const saleAmount=(s)=>{const cents=s?.amountCents??s?.amount_cents;if(cents!=null&&Number.isFinite(Number(cents)))return Number(cents)/100;const a=Number(s?.amount!=null?s.amount:s?.total);return Number.isFinite(a)?a:0;};
  const lineQty=(l)=>Math.max(0,+(l?.qty??l?.q)||0);
  function lineTotal(line,qty,fallbackPrice){const total=Number(line?.total??line?.t),unit=Number(line?.unitPrice??line?.unit??line?.u??line?.price);return Number.isFinite(total)&&total>=0?total:(Number.isFinite(unit)&&unit>=0?unit*qty:(+fallbackPrice||0)*qty);}
  /* Les ventes de la période, une fois chacune.
   * · annulées (void) : hors des livres, jamais comptées ;
   * · même règlement arrivé sous plusieurs ids (même ticket, même
   *   milliseconde, même montant, même panier) : compté une fois, et le
   *   doublon écarté est déclaré — voir KiwiDayReport.settlementKey. */
  function windowSales(w){
    const key=window.KiwiDayReport?.settlementKey,seenId=new Set(),seenKey=new Set(),out=[];let dupN=0,dupAmt=0;
    sales().forEach(s=>{
      const ts=+s?.ts||0;if(!s||ts<w.from||ts>=w.to||s.void_ts||s.voided)return;
      /* Le tableau de bord range l'id serveur dans `saleId` ; la caisse et les
         tests dans `id`. Le curseur ne suffit pas : un même règlement peut
         occuper deux lignes du registre. */
      const id=String(s.saleId||s.id||(s.cursor?'c'+s.cursor:''));if(id){if(seenId.has(id))return;seenId.add(id);}
      const k=key?key({id,ref:s.receiptRef||s.ref,ts,amount:saleAmount(s),lines:s.lines}):'';
      if(k&&seenKey.has(k)){dupN++;dupAmt+=saleAmount(s);return;}
      if(k)seenKey.add(k);out.push(s);
    });
    return{rows:out,dupN,dupAmt};
  }
  /* Les remboursements vivent dans leur propre registre (KiwiRefunds) : le
     pont du flux y range toute ligne serveur de montant négatif. */
  function windowRefunds(w){
    let list=[];try{list=window.KiwiRefunds?.list?.(venue())||[];}catch(_){list=[];}
    const seen=new Set();let n=0,amount=0;
    list.forEach(r=>{const ts=+r?.ts||0;if(!r||ts<w.from||ts>=w.to||r.void_ts||r.voided)return;const id=String(r.saleId||r.id||r.cursor||'');if(id){if(seen.has(id))return;seen.add(id);}n++;amount+=Math.abs(saleAmount(r));});
    return{n,amount};
  }
  function performanceData(days=30){
    const w=analysisWindow(days),items=D().items.map(x=>({item:x,qty:0,revenue:0,unitCost:null,unitProfit:null,profit:null,marginPct:null,quadrant:null,costSource:null}));
    const match=saleLineMatcher(items),ws=windowSales(w),offMenu=new Map();
    const ledger={grossN:0,gross:0,refundN:0,refunds:0,net:0,itemised:0,attributed:0,offMenu:0,freeAmount:0,unitemisedN:0,dupN:ws.dupN,dupAmt:ws.dupAmt,completeness:feedCompleteness()};
    ws.rows.forEach(s=>{
      const amount=saleAmount(s),refund=s.kind==='refund'||amount<0;
      if(refund){ledger.refundN++;ledger.refunds+=Math.abs(amount);return;}
      ledger.grossN++;ledger.gross+=amount;
      const lines=Array.isArray(s.lines)?s.lines:[];
      if(!lines.length){ledger.unitemisedN++;return;}
      lines.forEach(line=>{
        const qty=lineQty(line);if(!qty)return;
        const row=match(line);
        if(row){const v=lineTotal(line,qty,row.item.price);row.qty+=qty;row.revenue+=v;ledger.itemised+=v;ledger.attributed+=v;return;}
        /* Hors de la carte actuelle : article libre, plat retiré ou renommé,
           nom ambigu sans id. Jamais jeté, jamais rattaché au hasard. */
        const id=String(line?.itemId??line?.i??line?.id??''),name=String(line?.baseName||line?.bn||line?.name||line?.n||'').trim();
        const free=id==='libre'||/^article libre$/i.test(name);
        const k=free?'libre':(id?'id:'+id:'name:'+norm(name));
        const o=offMenu.get(k)||{key:k,id,name:free?ui('freeAmountItem'):(name||ui('article')),free,qty:0,revenue:0};
        const v=lineTotal(line,qty,0);o.qty+=qty;o.revenue+=v;offMenu.set(k,o);ledger.itemised+=v;ledger.offMenu+=v;if(free)ledger.freeAmount+=v;
      });
    });
    const rf=windowRefunds(w);ledger.refundN+=rf.n;ledger.refunds+=rf.amount;
    ledger.net=ledger.gross-ledger.refunds;
    /* Remises au ticket, frais de service, ventes sans détail : ce qui sépare
       le total encaissé de la somme des lignes. Affiché, jamais réparti. */
    ledger.unitemised=ledger.gross-ledger.itemised;
    ...
    /* `totalRevenue` = le CA des plats de la carte, seul périmètre où une
       recette peut exister. Ce n'est PAS le chiffre d'affaires du commerce :
       celui-ci est `ledger.gross` / `ledger.net`. */
    ...
    return {items,measured,medQty,medProfit,totalRevenue,costedRevenue,grossProfit,coverage:totalRevenue?costedRevenue/totalRevenue*100:0,days,window:w,ledger,offMenu:[...offMenu.values()].sort((a,b)=>b.revenue-a.revenue)};
  }
  function perfQuads(){
    return {
  ...
  /* Le registre de la période, AVANT l'analyse par plat : ventes brutes,
     remboursements, net, puis la part que les recettes peuvent expliquer.
     Chaque écart entre ces chiffres est nommé ; aucun sous-ensemble n'est
     présenté comme le chiffre d'affaires du commerce. */
  function perfLedger(p){
    const L=p.ledger,share=(v)=>L.gross>0?Math.round(v/L.gross*100)+' %':'·';
    const notes=[];
    if(L.completeness==='loading')notes.push(`<p class="warn">${esc(ui('ledgerLoading'))}</p>`);
    else if(L.completeness==='unknown')notes.push(`<p class="warn">${esc(ui('ledgerUnknown'))}</p>`);
    if(L.dupN)notes.push(`<p>${esc(ui('ledgerDuplicates',{n:L.dupN,amount:cash(L.dupAmt)}))}</p>`);
    if(L.offMenu)notes.push(`<p>${esc(ui('ledgerOffMenu',{amount:cash(L.offMenu),free:cash(L.freeAmount)}))}</p>`);
    if(Math.abs(L.unitemised)>=0.01)notes.push(`<p>${esc(ui('ledgerUnitemised',{amount:cash(L.unitemised),n:L.unitemisedN}))}</p>`);
  ...
  function performancePanel(){
    ...
    const offRows=p.offMenu.map(o=>`<tr class="rmw-perf-off"><td><b>${esc(o.name)}</b><small>${esc(o.free?ui('freeAmountSection'):ui('offMenuSection'))}</small></td><td>${o.qty.toLocaleString('fr-FR')}</td><td>${cash(o.revenue)}</td><td>·</td><td>·</td><td>·</td><td>${esc(o.free?ui('freeAmountReadout'):ui('offMenuReadout'))}</td></tr>`).join('');
  ...
  /* L'heure d'une vente dans le fuseau du COMMERCE, pas du navigateur : un
     tableau de bord ouvert depuis Paris rangeait le coup de feu de 21 h
     marocain dans la case 22 h. */
  let zoneClock=null,zoneClockZone='';
  function merchantMinute(ts){
    const zone=(()=>{try{return window.KiwiDayReport?.timezone?.()||'';}catch(_){return '';}})();
    if(!zone){const d=new Date(ts);return d.getHours()*60+d.getMinutes();}
    if(!zoneClock||zoneClockZone!==zone){try{zoneClock=new Intl.DateTimeFormat('en-GB',{timeZone:zone,hour:'2-digit',minute:'2-digit',hourCycle:'h23'});zoneClockZone=zone;}catch(_){zoneClock=null;const d=new Date(ts);return d.getHours()*60+d.getMinutes();}}
    const parts=zoneClock.formatToParts(new Date(ts));let h=0,m=0;parts.forEach(x=>{if(x.type==='hour')h=+x.value;if(x.type==='minute')m=+x.value;});return (h%24)*60+m;
  }
  function minuteInService(ts,service){const plain=merchantMinute(ts),m=service.to>1440&&plain<service.to-1440?plain+1440:plain;return m>=service.from&&m<service.to;}
  ...
  function hoursData(periods,selected,days=30){
    const rows=D().items.map(x=>({item:x,qty:0,revenue:0,byPeriod:{}})),match=saleLineMatcher(rows),w=analysisWindow(days);
    windowSales(w).rows.forEach(s=>{
      const ts=+s.ts||0;if(s.kind==='refund'||saleAmount(s)<0)return;
      const service=periods.find(p=>minuteInService(ts,p));if(!service)return;
      (Array.isArray(s.lines)?s.lines:[]).forEach(line=>{
        const row=match(line),qty=lineQty(line);
        if(!row||!qty)return;
        row.byPeriod[service.id]=(row.byPeriod[service.id]||0)+qty;
        if(service.id===selected.id){row.qty+=qty;row.revenue+=lineTotal(line,qty,row.item.price);}
      });
    });
  ...
  function hoursPanel(){
    ...
    const recent=configured?[]:windowSales(analysisWindow(30)).rows.filter(s=>s.lines?.length);
...
<div class="rmw-formula-builder" data-formula-wrap ${formulaSlots.length?'':'hidden'} style="margin-top:10px;display:grid;gap:10px;"><div data-slots-wrap style="display:grid;gap:10px;"></div><button type="button" class="btn-slim" data-add-slot style="justify-content:center;">${ic('plus')} ${esc(ui('addStepBtn'))}</button>${archivedItems.length?`<details class="rmw-archived-group"><summary>${esc(ui('archivedGroup'))} · ${archivedItems.length}</summary>${archivedItems.map(it=>`<button type="button" disabled>${esc(t(it.name))}</button>`).join('')}</details>`:''}</div></div>`,foot:(x?`<button class="kb ghost danger" data-archive>${esc(x.archived?ui('restore'):ui('archive'))}</button>`:'')+`<button class="kb ghost" data-cancel>${esc(ui('cancelBtn'))}</button><button class="eq-cta-gradient" data-save>${esc(ui('saveBtn'))}</button>`});m.el.classList.add('rmw-item-workspace');const isFormulaCb=$('[data-is-formula]',m.el),formulaWrap=$('[data-formula-wrap]',m.el),slotsWrap=$('[data-slots-wrap]',m.el);const renderSlots=()=>{if(!slotsWrap)return;slotsWrap.innerHTML=formulaSlots.map(slotRowHtml).join('')||'<span class="kf-help">Aucune étape.</span>';};
  ...
  function recipeSummary(x,draft,api,hide){const m=api.metrics(x,draft,venue()),price=+x.price||0,pending=(l)=>`<span class="rmw-pending">${esc(l)}</span>`,gain=m.profit==null?pending('À compléter'):`${m.profit.toFixed(2)} MAD`,actual=m.actualCost==null?pending('À mesurer'):`${m.actualCost.toFixed(2)} MAD`;let cls='neutral',title='Recette à compléter',detail='Ajoutez les quantités et reliez les ingrédients au stock.';if(m.costComplete&&m.actualCost==null){cls='ok';title='Recette chiffrée';detail='Le coût réel se comparera à cette fiche après les premières consommations.';}if(m.actualCost!=null){const gap=Math.abs(m.variancePct||0),bad=gap>15;cls=bad?'danger':gap>5?'warn':'ok';title=bad?'Écart à vérifier':gap>5?'Coût à surveiller':'Coût conforme';detail=`${gap.toFixed(0)} % ${m.variancePct>0?'au-dessus':'sous'} du coût prévu · contrôlez les portions et sorties de stock.`;}const n=api.nutrition?.(draft,venue()),hasLines=(draft.ingredients||[]).some(l=>+l.qty>0),missing=n?[...new Set([...(n.missingNutrition||[]),...(n.missingConversion||[]),...(n.missingAllergens||[])])]:[];const toggle=`<label class="rmw-nutri-toggle" title="${esc(ui('nutritionHideHint'))}"><input type="checkbox" data-hide-nutrition${hide?' checked':''}/><span>${esc(ui('nutritionHide'))}</span></label>`;let nutrition;if(!hasLines)nutrition=`<div class="rmw-nutrition-state empty"><div class="rmw-nutri-head"><span class="rmw-nutri-dot mode"><i></i>${esc(ui('nutritionShort'))}</span><span class="rmw-nutri-muted">${esc(ui('nutritionAwaitsRecipe'))}</span></div>${toggle}</div>`;else if(n?.complete)nutrition=`<div class="rmw-nutrition-state complete"><div class="rmw-nutri-head"><span class="rmw-nutri-dot req"><i></i>${esc(ui('nutritionComplete'))}</span>${toggle}</div><div class="rmw-nutrition-values"><bdi dir="ltr">${n.nutrition.kcal} kcal</bdi><bdi dir="ltr">P ${n.nutrition.protein} g</bdi><bdi dir="ltr">G ${n.nutrition.carbs} g</bdi><bdi dir="ltr">L ${n.nutrition.fat} g</bdi></div><div class="rmw-nutrition-allergens"><b>${esc(ui('nutritionAllergens'))}</b><span>${esc(n.allergens.length?n.allergens.map(key=>ui(`allergen_${key}`)).join(' · '):ui('nutritionNone'))}</span></div><small>${esc(ui('nutritionIndicative'))}</small></div>`;else nutrition=`<div class="rmw-nutrition-state incomplete"><div class="rmw-nutri-head"><span class="rmw-nutri-dot opt"><i></i>${esc(ui('nutritionIncomplete'))}</span><span class="rmw-nutri-muted">${esc(ui('nutritionMissing',{n:missing.length}))}</span></div><div class="rmw-nutri-head-acts"><button class="btn-slim" type="button" data-nutrition-fix>${esc(ui('nutritionFix'))}</button>${toggle}</div></div>`;return `<div class="rmw-recipe-state ${cls}"><i></i><div><b>${title}</b><span>${detail}</span></div></div><div class="rmw-recipe-costs"><div><span>Prix de vente</span><b>${price.toFixed(2)} MAD</b></div><div><span>Coût théorique</span><b>${m.costComplete?`${m.theoreticalCost.toFixed(2)} MAD`:'À compléter'}</b></div><div><span>Coût observé</span><b>${actual}</b><small>${m.actualCost==null?'Après ventes + suivi stock':`${price?((m.actualCost/price)*100).toFixed(1):0} % du prix`}</small></div><div class="total"><span>${m.actualCost==null?'Marge théorique':'Marge observée'}</span><b>${gain}</b><small>par portion</small></div></div>${nutrition}`;}
  function openRecipe(x){const api=window.KiwiRestaurantRecipes,r=api?.get(x.id,x.name,venue())||{portions:1,prepMinutes:0,ingredients:[],steps:[],note:''},inv=api?.inventory(venue())||[];const rows=r.ingredients.length?r.ingredients:[{stockId:'',name:'',qty:0,unit:''}];const m=modal({title:`Recette · ${esc(t(x.name))}`,desc:`${esc(cat(x.catId)?.name?t(cat(x.catId).name):'')}`+` · ${cash(x.price)} prix de vente`,width:940,body:`<div class="rmw-recipe-editor"><datalist id="rmw-stock-ingredients">${inv.map(s=>`<option value="${esc(s.name)}">${esc(s.unit||'')} · ${cash(s.costPerUnit)}</option>`).join('')}</datalist><div data-recipe-summary>${recipeSummary(x,r,api,!!x.hideNutrition)}</div><section class="rmw-recipe-section rmw-recipe-composition"><div class="rmw-recipe-section-head"><div><span>Fiche technique</span><h4>Composition</h4><p>Quantités par recette : elles pilotent le coût, la marge, le stock et la nutrition.</p></div><div class="rmw-recipe-top"><label><span>Portions</span><input class="kf-input" type="number" min="1" step="1" data-portions value="${r.portions||1}"/></label><label><span>Préparation</span><span class="rmw-recipe-suffix"><input class="kf-input" type="number" min="0" step="1" data-minutes value="${r.prepMinutes||0}"/><b>min</b></span></label></div></div><div class="rmw-recipe-ing-head"><span>Ingrédient</span><span>Quantité / recette</span><span>Unité</span><span></span></div><div data-recipe-ingredients>${rows.map(recipeIngredientRow).join('')}</div><button class="btn-slim rmw-recipe-add" type="button" data-ing-add>+ Ajouter un ingrédient</button>${inv.length?'':'<p class="kf-help">Ajoutez d’abord vos matières premières dans Stock &amp; approvisionnement pour calculer leur coût.</p>'}</section><div class="rmw-recipe-notes"><section class="rmw-recipe-section"><span class="rmw-recipe-kicker">Exécution</span><h4>Étapes de préparation</h4><textarea class="kf-input rmw-textarea" data-steps placeholder="Une étape par ligne">${esc((r.steps||[]).join('\n'))}</textarea></section><section class="rmw-recipe-section"><span class="rmw-recipe-kicker">Transmission</span><h4>Notes d’équipe</h4><textarea class="kf-input rmw-textarea" data-note placeholder="Allergènes, calibrage, dressage…">${esc(r.note||'')}</textarea></section></div></div>`,foot:`<button class="kb ghost" data-cancel>${esc(ui('cancelBtn'))}</button><button class="eq-cta-gradient" data-save>${esc(ui('saveBtn'))}</button>`});m.el.classList.add('rmw-recipe-modal');let hideDraft=!!x.hideNutrition;const wrap=$('[data-recipe-ingredients]',m.el),refresh=()=>{$('[data-recipe-summary]',m.el).innerHTML=recipeSummary(x,recipeDraft(m.el,x,api),api,hideDraft);};m.el.addEventListener('change',e=>{if(e.target.matches('[data-hide-nutrition]'))hideDraft=!!e.target.checked;});m.el.addEventListener('input',e=>{if(e.target.matches('[data-hide-nutrition]'))return;const row=e.target.closest('[data-recipe-ing]');if(row&&e.target.matches('[data-ing-name]')){const hit=inv.find(s=>String(s.name).trim().toLowerCase()===e.target.value.trim().toLowerCase());row.dataset.stockId=hit?.id||'';if(hit&&$('[data-ing-unit]',row))$('[data-ing-unit]',row).value=window.KiwiRestaurantUnits?.normalize?.(hit.unit)||hit.unit||'unité';}refresh();});m.el.addEventListener('click',e=>{if(!e.target.closest('[data-nutrition-fix]'))return;m.close();window.Kiwi?.handlers?.['nav-stock']?.();});wrap.onclick=e=>{const del=e.target.closest('[data-ing-delete]');if(!del)return;del.closest('[data-recipe-ing]').remove();if(!wrap.children.length)wrap.insertAdjacentHTML('beforeend',recipeIngredientRow({},0));refresh();};$('[data-ing-add]',m.el).onclick=()=>{wrap.insertAdjacentHTML('beforeend',recipeIngredientRow({},wrap.children.length));$('[data-ing-name]',wrap.lastElementChild).focus();};$('[data-cancel]',m.el).onclick=m.close;$('[data-save]',m.el).onclick=()=>{const draft=recipeDraft(m.el,x,api);if(!draft.ingredients.some(line=>line.qty>0)){toast('Quantités manquantes','Ajoutez au moins un ingrédient avec une quantité.','warn');return;}if(draft.ingredients.some(line=>!line.unit)){toast('Unités manquantes','Choisissez une unité pour chaque ingrédient.','warn');return;}S().updateItem(x.id,{hideNutrition:hideDraft});api.save(x.id,draft,venue());m.close();render();toast('Recette enregistrée','Stock, marges et nutrition mis à jour');};}
...
.rmw-nutrition-state.empty,.rmw-nutrition-state.incomplete{grid-template-columns:minmax(0,1fr) auto;align-items:center}.rmw-nutri-head{display:flex;flex-wrap:wrap;align-items:center;gap:6px 12px;min-width:0}.rmw-nutri-head .rmw-nutri-dot{font-weight:600;color:var(--ink)}.rmw-nutri-head-acts{display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:flex-end}.rmw-nutrition-state.complete .rmw-nutri-head{justify-content:space-between}.rmw-nutri-toggle{display:inline-flex;align-items:center;gap:7px;color:var(--n-500);font-size:11.5px;cursor:pointer;white-space:nowrap}.rmw-nutri-toggle input{width:15px;height:15px;margin:0;accent-color:var(--atlas)}.rmw-recipe-section-head p{max-width:60ch}
`;document.head.appendChild(s)}
...
.rmw-menu-search{width:min(100%,760px);min-height:54px;display:grid;grid-template-columns:38px minmax(0,1fr) auto;align-items:center;gap:11px;padding:7px 10px;border:1px solid var(--n-200);border-radius:16px;background:color-mix(in srgb,var(--surface) 92%,var(--paper-soft));box-shadow:0 1px 0 rgba(255,255,255,.04),0 12px 32px -30px rgba(0,0,0,.75);cursor:text;transition:border-color .16s ease,background .16s ease,box-shadow .16s ease,transform .16s ease}
.rmw-menu-search:hover{border-color:color-mix(in srgb,var(--atlas) 30%,var(--n-200));background:var(--surface)}
.rmw-menu-search:focus-within{border-color:color-mix(in srgb,var(--atlas) 72%,var(--n-200));background:var(--surface);box-shadow:0 0 0 3px color-mix(in srgb,var(--atlas) 13%,transparent),0 18px 42px -32px rgba(0,0,0,.75)}
.rmw-menu-search-icon{width:38px;height:38px;display:grid;place-items:center;border-radius:12px;background:color-mix(in srgb,var(--atlas) 10%,transparent);color:var(--atlas);transition:background .16s ease,transform .16s ease}
.rmw-menu-search:focus-within .rmw-menu-search-icon{background:color-mix(in srgb,var(--atlas) 16%,transparent);transform:scale(1.03)}
.rmw-menu-search-icon svg{width:19px;height:19px;fill:currentColor}
.rmw-menu-search input{min-width:0;width:100%;height:38px;padding:0;border:0!important;outline:0!important;background:transparent!important;color:var(--ink);font:550 14px var(--sans);letter-spacing:-.005em;box-shadow:none!important;-webkit-appearance:none;appearance:none}
.rmw-menu-search input::placeholder{color:var(--n-500);opacity:.8}
.rmw-menu-search input::-webkit-search-cancel-button{width:18px;height:18px;margin-inline:8px 2px;cursor:pointer;opacity:.62}
.rmw-menu-search-count{min-width:66px;display:grid;grid-template-columns:auto auto;align-items:baseline;justify-content:end;gap:4px;padding:7px 9px;border-radius:10px;background:var(--paper-soft);color:var(--n-500);white-space:nowrap;transition:background .16s ease,color .16s ease}
.rmw-menu-search-count strong{color:var(--ink);font:700 11px var(--mono)}
.rmw-menu-search-count small{font:600 9px var(--mono);letter-spacing:.02em}
.rmw-menu-search.has-query .rmw-menu-search-count{background:color-mix(in srgb,var(--atlas) 10%,transparent);color:var(--atlas)}
.rmw-menu-search.has-query .rmw-menu-search-count strong{color:var(--atlas)}
@media(max-width:620px){.rmw-menu-search{width:100%;grid-template-columns:36px minmax(0,1fr);min-height:50px;padding:6px 9px;border-radius:14px}.rmw-menu-search-icon{width:36px;height:36px}.rmw-menu-search-count{display:none}}
`;document.head.appendChild(s)}
...
`;document.head.appendChild(s);}
  function directCardAction(e){const control=e.target.closest?.('[data-rmw-command]');if(control){e.preventDefault();e.stopPropagation();const id=control.dataset.arg,x=item(id),command=control.dataset.rmwCommand;control.closest('details')?.removeAttribute('open');if(command==='availability'&&x&&!x.archived){S().updateItem(id,{avail:x.avail===false});render();}else if(command==='archive'&&x){S().updateItem(id,{archived:!x.archived});render();}else if(command==='delete'&&x){confirm(ui('deleteItemConfirm',{name:x.name}),()=>{S().deleteItem(id);render();});}else if(command==='duplicate'){const before=new Set(D().items.map(it=>it.id));S().duplicateItem(id);const copy=D().items.find(it=>!before.has(it.id));if(copy)openItem(copy);}return;}if(e.target.closest?.('.rmw-card-menu'))return;const card=e.target.closest?.('[data-rmw-card-edit]');if(card){e.preventDefault();e.stopPropagation();openItem(item(card.dataset.rmwCardEdit));}}
  let booted=false,storeBound=false,uiBound=false;
  function boot(){
    if(!window.Kiwi?.handlers){setTimeout(boot,50);return;}
    if(!booted){booted=true;style();nutritionStyle();performanceStyle();hoursStyle();bind();}
  ...
  bind();
  document.addEventListener('pointerdown',directCardAction,true);
  document.addEventListener('keydown',e=>{if((e.key==='Enter'||e.key===' ')&&e.target.matches?.('[data-rmw-card-edit]'))directCardAction(e);},true);
