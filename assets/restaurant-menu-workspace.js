  ...
  function openItem(x){
    ...
    const slotRowHtml=(slot,si)=>{
      ...
      if(availableItems.length===0){
        ...
        addBarHtml=`<div class="rmw-slot-add-bar"><select class="kf-input" data-pick-item="${si}" data-f-slot-pick="${si}"><option value="">${esc(ui('chooseItemPlaceholder'))}</option>${addSelect}</select><button type="button" class="btn-slim" data-add-choice="${si}">${ic('plus')} ${esc(ui('addChoiceBtn'))}</button></div>`;
      }
