...
(function () {
  ...
  function card(x){
    ...
    const actionMenu=`<span class="rmw-card-menu" data-rmw-menu-root><button class="rmw-more-btn" data-action="rmw-item-more" data-arg="${x.id}" aria-label="Plus d’actions" aria-haspopup="menu" aria-expanded="${menuOpen}">${ic('more')}</button>${menuOpen?`<span class="rmw-action-pop" role="menu">${x.formula?`<button data-action="rmw-formula-duplicate" data-arg="${x.id}" role="menuitem">${ic('plus')}<span>${esc(stateUi('reuse','Réutiliser'))}</span></button>`:''}<button data-action="rmw-item-archive" data-arg="${x.id}" role="menuitem">${ic(x.archived?'restore':'archive')}<span>${esc(archiveLabel)}</span></button><button class="danger" data-action="rmw-item-delete" data-arg="${x.id}" role="menuitem">${ic('trash')}<span>${esc(stateUi('delete','Supprimer'))}</span></button></span>`:''}</span>`;
    return `<article class="mi-card${x.avail===false?' rmw-off':''}${x.archived?' rmw-archived':''}${menuOpen?' rmw-menu-open':''}" data-action="rmw-item-edit" data-arg="${x.id}" role="button" tabindex="0" aria-label="${esc(ui('editItemTitle',{name:xName}))}">${media}<div class="mi-card-top"><span class="mi-card-cat" title="${esc(cName+(subName?` · ${subName}`:''))}">${esc(subName||cName)}</span>${formulaBadge}${badges}${actionMenu}</div><div class="mi-card-name">${esc(xName)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(sName)}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`}</span><span class="mi-card-acts"><button class="mi-state-toggle" data-action="rmw-item-avail" data-arg="${x.id}" role="switch" aria-checked="${x.avail!==false}"${x.archived?' disabled':''}>${x.avail===false?'86':'Disponible'}</button></span></div></article>`;
  }
