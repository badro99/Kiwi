...
(function () {
  ...
  function openItem(x){
    ...
    const slotRowHtml=(slot,si)=>{
      ...
      return `<div class="rmw-slot-card" data-slot-idx="${si}"><div class="rmw-slot-head"><div class="rmw-slot-title"><span class="rmw-slot-kicker">${esc(ui('step'))} ${si+1}</span><input class="kf-input" data-slot-label="${si}" value="${esc(slot.label||'')}" placeholder="${esc(ui('stepTitlePlaceholder'))}"/></div><div class="rmw-slot-bounds" role="group" aria-label="${esc(ui('selectionRule'))}"><label><span>${esc(ui('minLabel'))}</span><input class="kf-input" type="number" min="0" max="10" data-slot-min="${si}" value="${slot.min!=null?slot.min:1}"/></label><label><span>${esc(ui('maxLabel'))}</span><input class="kf-input" type="number" min="1" max="10" data-slot-max="${si}" value="${slot.max!=null?slot.max:1}"/></label></div><button type="button" class="mi-ic-btn danger rmw-slot-delete" data-del-slot="${si}" aria-label="${esc(ui('deleteStep'))}">${ic('trash')}</button></div><div class="rmw-slot-choices">${choicesHtml||`<span class="kf-help">${esc(ui('noChoicesInStep'))}</span>`}</div>${addBarHtml}</div>`;
    };
