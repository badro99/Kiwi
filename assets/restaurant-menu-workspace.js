...
(function () {
  'use strict';
  let tab='menu', filter='all', subFilter=null, query='', hoursPeriod=null; // subFilter : sous-catégorie active ('__none' = non classés)
  ...
  function shownItems(){const q=query.trim().toLowerCase();return D().items.filter(x=>(filter==='all'||x.catId===filter)&&(filter==='all'||!subFilter||(subFilter==='__none'?!x.subId:x.subId===subFilter))&&(!q||`${x.name} ${x.desc||''}`.toLowerCase().includes(q)));}
  const subsOf=(cid)=>cat(cid)?.sub||[];
  /* Rangée des sous-catégories de la section ouverte — 36 sweets en une grille
   * plate ne se lisent pas ; Cookies / Gâteaux / Cheesecakes si. */
  function subChips(){if(filter==='all')return '';const subs=subsOf(filter),all=D().items.filter(x=>x.catId===filter);const none=all.filter(x=>!x.subId).length;const chip=(id,l,on)=>`<button class="mi-subchip${on?' on':''}" data-action="rmw-sub-filter" data-sub="${id}">${l}</button>`;let h=chip('',`Tout · ${all.length}`,!subFilter)+subs.map(s=>chip(s.id,`${esc(s.name)} · ${all.filter(x=>x.subId===s.id).length}`,subFilter===s.id)).join('');if(subs.length&&none)h+=chip('__none',`À classer · ${none}`,subFilter==='__none');h+=`<button class="mi-subchip add" data-action="rmw-sub-add" data-arg="${filter}">+ Sous-catégorie</button>`;if(subs.length&&none)h+=`<button class="mi-subchip add" data-action="rmw-classify" data-arg="${filter}">${ic('edit')} Classer ${none} article${none>1?'s':''}</button>`;if(subFilter&&subFilter!=='__none'&&subs.some(s=>s.id===subFilter)){h+=`<button class="mi-subchip add" data-action="rmw-sub-rename" data-arg="${filter}::${subFilter}">${ic('edit')} Renommer</button><button class="mi-subchip add danger" data-action="rmw-sub-delete" data-arg="${filter}::${subFilter}">${ic('trash')} Supprimer</button>`;}return `<div class="mi-subchips">${h}</div>`;}
  /* Grille groupée par sous-catégorie — un bandeau par groupe, les non classés
   * en fin de page. Grille plate quand il n'y a rien à grouper. */
  function gridHtml(list){const flat=()=>`<div class="mi-grid">${list.map(card).join('')}</div>`;if(filter==='all'||subFilter||query.trim())return flat();const subs=subsOf(filter);if(!subs.length)return flat();const groups=subs.map(s=>({id:s.id,label:s.name,items:list.filter(x=>x.subId===s.id)})).filter(g=>g.items.length);const rest=list.filter(x=>!x.subId||!subs.some(s=>s.id===x.subId));if(rest.length)groups.push({id:'__none',label:'Sans sous-catégorie',items:rest,muted:true});if(groups.length<2)return flat();return groups.map(g=>`<div class="mi-sub-head${g.muted?' muted':''}" data-action="rmw-sub-filter" data-sub="${g.id}" role="button" tabindex="0"><span>${esc(g.label)}</span><small>${g.items.length} article${g.items.length>1?'s':''}</small></div><div class="mi-grid">${g.items.map(card).join('')}</div>`).join('');}
  /* Classement en masse — tous les articles non classés de la section sur un
   * seul écran, un select par article, une écriture par changement. */
  function openClassify(cid){const subs=subsOf(cid);if(!subs.length){toast('Créez d’abord une sous-catégorie','Utilisez « + Sous-catégorie ».','info');return;}const pending=D().items.filter(x=>x.catId===cid&&!x.subId);if(!pending.length){toast('Tout est classé','Aucun article sans sous-catégorie.','info');return;}const opts=subs.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('');const m=modal({tag:'CLASSEMENT',title:`Classer les articles · ${esc(cat(cid)?.name||'')}`,width:560,body:`<div class="kf-help" style="margin-bottom:10px;">${pending.length} article${pending.length>1?'s':''} sans sous-catégorie. Chaque choix est enregistré immédiatement.</div><div class="mi-classify-list">${pending.map(x=>`<div class="mi-classify-row" data-mcr="${x.id}"><span class="mi-classify-name">${esc(x.name)}</span><select class="kf-input" data-rmw-classify-sub="${x.id}"><option value="">— Choisir —</option>${opts}</select></div>`).join('')}</div>`,foot:'<button class="eq-cta-gradient" data-done>Terminé</button>'});m.el.addEventListener('change',e=>{const sel=e.target.closest('[data-rmw-classify-sub]');if(!sel||!sel.value)return;S().updateItem(sel.dataset.rmwClassifySub,{subId:sel.value});sel.closest('[data-mcr]')?.classList.add('done');});$('[data-done]',m.el).onclick=()=>{m.close();render();};}
  function card(x){const c=cat(x.catId),sid=c?.station||S().kitchenId(venue()),s=station(sid),sub=c&&x.subId?(c.sub||[]).find(k=>k.id===x.subId):null;const media=x.video?`<video class="rmw-media" src="${esc(x.video)}" muted playsinline preload="metadata"></video>`:x.photo?`<img class="rmw-media" src="${esc(x.photo)}" alt=""/>`:'';const formulaBadge=x.formula?`<span class="mi-tag" style="background:var(--mint-tint);color:var(--forest-ink);font-weight:700;margin-left:4px;">FORMULE</span>`:'';return `<article class="mi-card${x.avail===false?' rmw-off':''}" data-action="rmw-item-edit" data-arg="${x.id}">${media}<div class="mi-card-top"><span class="mi-card-cat">${esc(c?.name||'Sans section')}${sub?` · ${esc(sub.name)}`:''}</span>${formulaBadge}${x.avail===false?'<span class="mi-tag">86</span>':''}</div><div class="mi-card-name">${esc(x.name)}</div><div class="mi-card-price-row"><span class="mi-card-price">${cash(x.price)}</span><span class="mi-card-station">→ ${esc(s?.name||'Cuisine')}</span></div><div class="mi-card-foot"><span>${x.formula?`${(x.formula.slots||[]).length} étape(s)`:`${(x.opts||[]).length} groupe(s) d’options`}</span><span class="mi-card-acts">${x.formula?`<button class="btn-slim" data-action="rmw-formula-duplicate" data-arg="${x.id}">${ic('plus')} Réutiliser</button>`:''}<button class="mi-ic-btn" data-action="rmw-item-edit" data-arg="${x.id}">${ic('edit')}</button><button class="mi-ic-btn danger" data-action="rmw-item-delete" data-arg="${x.id}">${ic('trash')}</button></span></div></article>`;}
